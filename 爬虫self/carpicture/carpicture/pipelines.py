...
class SaveImagePipeline(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        ...
        filename = 'carpicture/{}/{}/{}'.format(name, title, image_guid)
        return filename
...
class CarnamePipeline(object):
    def __init__(self):
        self.save_dir = 'C:/Users/YJY/Desktop/workspace/carpicture'

    def save_file(self, item, spider):
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
        path = ''.join([self.save_dir, '/', 'keywords2.txt'])
        with open(path, 'w', encoding='utf-8') as f:
            for item in item['title']:
                f.write(item + '\t')
